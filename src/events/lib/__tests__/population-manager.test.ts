...
describe('`Population` class', () => {
  ...
  describe('`depopulateRoom` method', () => {
    ...
    let rooms: t.IRooms
    let user1: t.TUsername
    let user2: t.TUsername

    beforeEach(() => {
      rooms = p.rooms
      room = faker.random.word()
      user1 = faker.random.number().toString()
      user2 = faker.random.number().toString()
    ...
    it('should remove a user from a room', () => {
      ...
      const expected: t.IRoom = { leader: user1, users: [user1] }
      expect(rooms[room]).toEqual(expected)
    })
    ...
    it('should do nothing if the user is not in the room', () => {
      const user3: t.TUsername = faker.random.number().toString()
  ...
  describe('`details` method', () => {
    ...
    let user1: t.TSocketId
    let username1: t.TUsername
    let user2: t.TSocketId
    let username2: t.TUsername
    let user3: t.TSocketId
    ...
    beforeEach(() => {
      ...
      user3 = faker.random.number().toString()
  ...
  describe('`moveUser method`', () => {
    ...
    it('should add a user to a new room', () => {
      f.moveUser(user1, oldRoom, newRoom)
      const result: boolean = f.rooms[newRoom].users.includes(user1)
      expect(result).toBeTruthy()
    })

    it('should remove the user from the old room', () => {
      f.moveUser(user1, oldRoom, newRoom)
      const result: boolean = f.rooms[oldRoom].users.includes(user1)
      expect(result).toBeFalsy()
    })

    it('should not affect other users in the old room', () => {
      const result: boolean = f.rooms[oldRoom].users.includes(user2)
      expect(result).toBeTruthy()
    })
